...
uri = "mongodb+srv://dadumoni01:8lgG7Z3zDh6YYKui@cluster0.3wcnpfx.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
...
def link_gen(uname, long_link):
    ...
    if auth_info is not None:
        ...
        url = f"https://domain.com/api?api={api_key}&url={long_link}&format=text"
        headers = {
...
def start(update, context):
    keyboard = [
                [InlineKeyboardButton("Sign Up", url="https://domain.com/auth/signup")],
            ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    message_reply_text = '''😋This bot will help you to Short Links from your {Url Shortener Name} Account.

If you don't have an active {Url Shortener Name} Account then Please register your account here {Url Shortener Name}/auth/signup
...
def help(update, context):
    keyboard = [
                [InlineKeyboardButton("Get Help", url="https://domain.com/member/forms/support")],
            ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
def feature(update, context):
    update.message.reply_text("""💠 Features Of {Url Shortener Name} bot 💠

❤️ It's AN AI Based User Friendly Bot ❤️

➡️ Use Can Short Bulk Links Into Your {Url Shortener Name} Account With This Bot""")
...
def get_api(update, context):
    keyboard = [
                [InlineKeyboardButton("Get Token", url="domain.com/member/tools/api")],
            ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    message_reply_text = """• First Visit the domain.com/member/tools/api
• Copy the API TOKEN and come back to Bot.
• Input  /token and Paste The token Copied from domain.com/member/tools/api
• Now bot will successfully connected to your {Url Shortener Name} account."""
    update.message.reply_text(message_reply_text, reply_markup=reply_markup)
...
def main():
    ...
    bot_token = "7155790776:AAEIuecf_yqZMx3MoYNIhhJ3wI0bVpab2_0"
